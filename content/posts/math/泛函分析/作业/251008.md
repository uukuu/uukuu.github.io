...
(2) 构造映射 $T: l^\infty\to C(0,1]$, $\forall \lbrace a_n\rbrace \in l^\infty$,  
$T(\lbrace a_n\rbrace )$ 表示 $x(\frac1n)=a_n$ 且用折线段连接 $x(\frac 1n),x(\frac1{n+1})$ 两点的连续函数.  

显然这是个单射且像集是 $C(0,1]$ 的子集. 并且该映射是线性映射.  

下面证其还保范数: 对折线段, 其最大值取在端点处, 故  
$\Vert T(\lbrace a_n\rbrace )\Vert=\sup x(\frac 1 n)=\sup a_n=\Vert\lbrace a_n\rbrace \Vert$.  
